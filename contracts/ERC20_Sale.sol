...
contract ERC20Sale is Ownable, Pausable, ReentrancyGuard {
    ...
    modifier isWhitelistted(bytes32[] memory proof) {
        if (iswhitelist) {
            require(
    ...
    function getContractBalance() public view returns (uint256) {
        ...
        return totalBal;  //will return amount in 18 decimal 
    }
    ...
    function buyToken(
    ...
    ) external whenNotPaused nonReentrant isWhitelistted(proof) {
        ...
        uint256 tokensPurchased = ((_amount * 10**12) / priceInUSD) ;
        uint256 userUpdatedBalance = claimable[msg.sender] + tokensPurchased;
        require( _amount  + getContractBalance() <= (hardcap * CENTS), "Hardcap reached" );
        require((((claimable[msg.sender] * ((priceInUSD * 10**12) / CENTS)) / MULTIPLIER) * CENTS) + _amount  <= allowedUserBalance * CENTS,"Exceeded allowance");
    ...
    function doTransferIn(
        ...
        INonStandardERC20 _token = INonStandardERC20(USDT);
        uint256 balanceBefore = INonStandardERC20(USDT).balanceOf(
            address(this)
        );
        ...
        uint256 balanceAfter = INonStandardERC20(USDT).balanceOf(
            address(this)
        );
     ...
     */
    function withdrawUSDT( uint256 _value)
        external
        onlyOwner
        whenPaused
        nonReentrant
    {
        doTransferOut( TREASURY, _value);
    }
